...
Will try to look for the AppModule bootstrap array this helps the angular to construct the contents to bundel files at the end in the index.html files it will try to import. In this way it will try to load the page.
...
Components are the basic building blocks of the angular application. By spliting the individual section of the page to multiple components. We have the flexibility to reuse the component whenever we want. We have more modular code which is in return very easy to maintain.
